...
export default async function SharedRecipePage({ params }: { params: { id: string } }) {
  ...
  const share = await db.recipeShare.findFirst({ where: sharedRecipeWhere(user.id, params.id), include: { recipe: { include: { user: { select: { email: true, username: true } } } } } });
  if (!share) notFound();
  const recipe = recipeView(share.recipe);
  const discussion = await recipeDiscussion(recipe.id, user.id);
  return <>
    <Link className="back-link" href="/friends"><ArrowLeft size={16} />Back to your kitchen crew</Link>
    <div className="detail-hero"><RecipeArt vibe={recipe.vibe} imageUrl={recipe.imageUrl} title={recipe.title} /><div><Vibe vibe={recipe.vibe} /><h1>{recipe.title}</h1>{recipe.altTitle && <p className="detail-subtitle">{recipe.altTitle}</p>}<UpdatedDate date={recipe.updatedAt}/><p className="servings"><Users size={17} />{recipe.servings} servings <span>•</span>{recipe.ingredients.length} ingredients</p><p className="shared-author">From {displayUsername(share.recipe.user)}’s kitchen</p>{user.roastEnabled && recipe.roastLine && <p className="speech">🧑‍🍳 {recipe.roastLine}</p>}<p className="shared-readonly"><LockKeyhole size={15} />Shared with you · only the owner can edit</p></div></div>
    ...
    <RecipeDiscussion recipeId={recipe.id} viewerId={user.id} ingredients={recipe.ingredients.map(item => item.name)} discussion={discussion}/>
  </>;
}
