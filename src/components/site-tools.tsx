...
export function SiteTools() {
  ...
  useEffect(() => { const close=(event:KeyboardEvent)=>{if(event.key==='Escape')setMenu(false);};addEventListener('keydown',close);return()=>removeEventListener('keydown',close); }, []);
  ...
  return <><a href="#main" className="skip-link">Skip to content</a><header className="site-header"><Link href="/recipes" className="header-brand">Recipe Buddy<span>YOUR DAILY DISH OF INSPIRATION</span></Link><form action="/search" role="search" className="global-search"><Search size={17} /><label className="sr-only" htmlFor="site-search">Search recipes and help</label><input id="site-search" name="q" maxLength={100} placeholder="Find a recipe, ingredient or answer…" /><button type="submit" aria-label="Search site">Go</button></form><button className="icon-button" onClick={theme} aria-label={dark ? 'Switch to light mode' : 'Switch to dark mode'} aria-pressed={dark}>{dark ? <Sun size={19} /> : <Moon size={19} />}</button><button className="icon-button mobile-menu-toggle" aria-expanded={menu} aria-controls="mobile-navigation" onClick={() => setMenu(!menu)} aria-label={menu ? 'Close menu' : 'Open menu'}>{menu ? <X size={20} /> : <Menu size={20} />}</button><div ref={bar} className="scroll-progress" aria-hidden="true" /></header>{menu && <nav id="mobile-navigation" className="mobile-navigation" aria-label="Mobile navigation">{[['/recipes','My recipes'],['/shopping-list','Shopping list'],['/friends','Friends'],['/settings','Settings'],['/help','Help & FAQ'],['/search','Search everything']].map(([href,label]) => <Link href={href} key={href} onClick={() => setMenu(false)}>{label}</Link>)}<button onClick={() => signOut({callbackUrl:'/login'})}>Sign out</button><button onClick={() => setMenu(false)}>Close menu</button></nav>}<div className="floating-tools">{top && <button className="icon-button" aria-label="Back to top" onClick={() => { window.scrollTo({top:0,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'}); document.getElementById('main')?.focus({preventScroll:true}); }}><ArrowUp size={20} /></button>}<Link href="mailto:contact@recipebuddy.waelfz.com?subject=Recipe%20Buddy%20feedback" className="button primary" aria-label="Contact Recipe Buddy"><Mail size={18} /><span>Get in touch</span></Link></div>{cookies && <aside className="cookie-banner" aria-label="Cookie notice"><div><strong>A small cookie, no crumbs.</strong><p>We use essential cookies to keep you signed in, and local storage for preferences. No advertising cookies.</p></div><button className="button primary" onClick={() => { setCookies(false); try { localStorage.setItem('rb-cookie-notice-v1','seen'); } catch { /* Dismiss for this visit. */ } }}>Got it</button></aside>}</>;
}
