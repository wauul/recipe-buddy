...
export function AuthForm({ signup = false }: { signup?: boolean }) {
  const [visible,setVisible] = useState(false); const router = useRouter(); const [error, setError] = useState(''), [busy, setBusy] = useState(false);
  ...
  return <main id="main" tabIndex={-1} className="auth-page"><div className="auth-story"><Link href="/" className="brand"><span className="brand-icon"><ChefHat /></span>recipe<span className="brand-light">buddy</span></Link><div><span className="eyebrow">YOUR KITCHEN, BUT A LITTLE MORE FUN</span><h1>Good food.<br />Questionable<br /><em>chef jokes.</em></h1><p>A cozy home for your recipes, your shopping lists,<br />and your next “actually, I made this” moment.</p><div className="auth-food" aria-hidden="true">🍝<span>✳</span></div></div><small>NO FANCY EQUIPMENT. JUST YOU AND AN APPETITE.</small></div><div className="auth-form-wrap"><form onSubmit={submit} className="auth-form"><span className="eyebrow">PULL UP A CHAIR</span><h2>{signup ? 'Join the kitchen.' : 'Welcome back, chef.'}</h2><p>{signup ? 'Your recipe collection deserves a home.' : 'Something delicious is waiting for you.'}</p><label>Email address<input name="email" type="email" autoComplete="email" required maxLength={254} placeholder="chef@example.com" /></label><label>Password<span className="password-field"><input name="password" type={visible ? 'text' : 'password'} autoComplete={signup ? 'new-password' : 'current-password'} required minLength={8} maxLength={72} placeholder="At least 8 characters" /><button type="button" aria-label={visible ? 'Hide password' : 'Show password'} aria-pressed={visible} onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={19}/> : <Eye size={19}/>}</button></span></label>{error && <p className="error" role="alert">{error}</p>}<button aria-busy={busy} disabled={busy} className="button primary">{busy ? 'Opening the kitchen…' : signup ? 'Create account' : 'Let’s get cooking'}<ArrowRight size={18} /></button><p className="auth-switch">{signup ? 'Already part of the kitchen?' : 'New around here?'} <Link href={signup ? '/login' : '/signup'}>{signup ? 'Log in' : 'Create an account'}</Link></p></form></div></main>;
}
