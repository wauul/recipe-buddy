...
export function RecipeDashboard({ recipes, progress }: { recipes: RecipeView[]; progress: Progress }) {
  ...
  return <>
    ...
    {filtered.length ? <div className="recipe-grid">{filtered.map(recipe => <Link className="recipe-card" href={`/recipes/${recipe.id}`} key={recipe.id}><div className="card-image"><RecipeArt vibe={recipe.vibe} imageUrl={recipe.imageUrl} title={recipe.title} /><Vibe vibe={recipe.vibe} /></div><div className="card-body"><h3>{recipe.title}</h3><p className="alt-title">{recipe.altTitle || 'A little homemade happiness.'}</p><UpdatedDate date={recipe.updatedAt}/><div className="card-bottom"><span><Users size={14} /> {recipe.servings} servings</span><span>{recipe.ingredients.length} ingredients <ArrowUpRight size={16} /></span></div></div></Link>)}</div> : <div className="empty-state"><div className="empty-illustration" aria-hidden="true">🥣<span>✦</span></div><h2>{recipes.length ? 'Nothing simmering here.' : 'Your next favorite starts here.'}</h2><p>{recipes.length ? 'Try another search or a different vibe.' : 'No recipes yet. Even instant noodles count, we don’t judge.'}</p>{!recipes.length && <Link href="/recipes/new" className="button primary"><Plus size={16} /> Add your first recipe</Link>}</div>}
