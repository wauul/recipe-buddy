...
export function FriendsDashboard({ friends, recipes }: { friends: FriendView[]; recipes: SharedRecipeView[] }) {
  ...
  return <>
    ...
    <section className="social-invite form-panel">
      ...
      <form onSubmit={invite}><label htmlFor="friend-email" className="sr-only">Friend’s email address</label><input id="friend-email" type="email" required maxLength={254} value={email} onChange={e => setEmail(e.target.value)} placeholder="your-favorite-chef@example.com" /><button className="button primary" disabled={!!busy}><UserPlus size={16} />{busy === 'invite' ? 'Sending…' : 'Add friend'}</button></form>
      <small>They’ll see your email and can accept in their Friends page. Adding a friend never shares recipes automatically.</small>
    </section>
    ...
    <div className="social-people">
      <section className="form-panel"><div className="social-section-title"><h2>Your sous-chefs</h2><span>{accepted.length}</span></div>
        {accepted.length ? <ul className="friend-list">{accepted.map(friend => <li key={friend.id}><div className="friend-person"><span className="avatar">{friend.friend.email[0].toUpperCase()}</span><div><strong>{friend.friend.email}</strong><small>Ready for your next recipe recommendation</small></div></div>
          {removing === friend.id ? <div className="friend-confirm"><p>Remove this friend? Shared recipe access ends for both of you.</p><div><button className="button secondary" disabled={!!busy} onClick={() => setRemoving('')}>Keep friend</button><button className="button danger" disabled={!!busy} onClick={() => change(friend, 'DELETE')}>Remove</button></div></div> : <button className="text-button" disabled={!!busy} onClick={() => setRemoving(friend.id)}>Remove friend</button>}
      ...
      <section className="form-panel"><div className="social-section-title"><h2>At the kitchen door</h2><span>{pending.length}</span></div>
        {pending.length ? <ul className="friend-list">{pending.map(friend => <li key={friend.id}><div className="friend-person"><span className="avatar">{friend.friend.email[0].toUpperCase()}</span><div><strong>{friend.friend.email}</strong><small>{friend.status === 'incoming' ? 'Wants to be your kitchen buddy' : 'Request sent · waiting for a yes'}</small></div></div><div className="friend-buttons">{friend.status === 'incoming' && <button className="button primary" disabled={!!busy} onClick={() => change(friend, 'PATCH')}><Check size={15} />Accept</button>}<button className="button secondary" disabled={!!busy} onClick={() => change(friend, 'DELETE')}><X size={15} />{friend.status === 'incoming' ? 'Decline' : 'Cancel request'}</button></div></li>)}</ul> : <div className="social-empty"><span aria-hidden="true">✉️</span><p>No pending requests.<br />A peaceful moment in the kitchen.</p></div>}
      </section>
    </div>
