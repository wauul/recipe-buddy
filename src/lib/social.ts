...
export async function friendList(id: string) {
  ...
  return rows.map(row => ({ id: row.id, friend: { ...(row.userAId === id ? row.userB : row.userA), username: displayUsername(row.userAId === id ? row.userB : row.userA) },
...
export async function sharedRecipes(id: string) {
  ...
  return rows.map(row => ({ ...recipeView(row.recipe), sharedBy: displayUsername(row.recipe.user) }));
}
